...
export class PayeeDetailComponent implements OnInit, OnChanges {
  ...
  /*
   * We need to implement a GoBack button
   *
   * Create a variable goBack;
   * Add an @Output() decorator to it
   *
   * Add an event handler, fireBack() which invokes emit() on goBack
   * No data payload, the event firing is enough
   *
   * Add the button to payee-detail.component.html
   */
