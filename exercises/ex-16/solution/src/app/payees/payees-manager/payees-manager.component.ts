...
export class PayeesManagerComponent implements OnInit {
  ...
  handleFilterPayees() {
    this.displayPayees = this.payees.filter(payee => payee.payeeName.toUpperCase().includes(this.filterPayeeName.toUpperCase()));
  }

  handleSortPayees(sortField) {

    this.displayPayees = sortBy( this.payees, sortField );
