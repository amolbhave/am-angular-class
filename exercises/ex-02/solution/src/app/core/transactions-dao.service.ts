...
export class TransactionsDAOService {
  ...
  params = {
    getLastTransaction: '_sort=txDate&_order=desc&_limit=1&_expand=payee',
    getLastFiveTransactions: '_sort=txDate&_order=desc&_limit=5&_expand=payee',
  };

  constructor( private client: HttpClient ) { }

  list(): Observable<Transaction[]> {
    return this.client.get<Transaction[]>( this.baseUrl );
  }

  get( id: string | number ) {
    return this.client.get<Transaction>( `${this.baseUrl}/${id}?_expand=category&_expand=payee` );
  }
